...
class APITestCase(TestCase):
    def setUp(self):
        ...
        '''
        # create test users:
        User.objects.create_user(username='rabih', email='rabih@britecore.com',
                                 password='adminadmin')
    ...
    def test_endpoint_url_risktypes_unauthenticated(self):
        '''
        This test makes sure that unauthentcated calls to endpoint receive
    ...
    def test_endpoint_url_risktypes_regular(self):
        '''
        This is a regular test for the endpoint which should return a nominal
        JSON response
        '''
        c = Client()
    ...
    def test_endpoint_url_riskinstance_new_unauthenticated(self):
        '''
        This test makes sure that unauthentcated calls to endpoint receive an
    ...
    def test_endpoint_url_riskinstance_new_empty(self):
        '''
        This tests that a a call to this endpoint without an argument
    ...
    def test_endpoint_url_riskinstances_regular(self):
        '''
        This tests that a a call to this endpoint with an argument returns a
        nominal JSON object
        '''
        c = Client()
        c.login(username='phil', password='adminadmin')
        response = c.get(reverse('url_riskinstances'), follow=True)
        nominal_response_content = b'''[{"id": 45, "title": "Automobile_7"}, {"id": 46, "title": "Automobile_8"}, {"id": 48, "title": "Property_10"}, {"id": 47, "title": "Property_9"}]'''  # @IgnorePep8
        self.assertEqual(response.status_code, 200)
        nominal_response_content = normalize_ids(nominal_response_content)
        response_content = normalize_ids(response.content)
        self.assertEqual(response_content, nominal_response_content)
    ...
    def test_endpoint_url_riskinstance_post_new(self):
        '''
        This tests that a a call to this endpoint creates
        a new risk instance
        '''
        c = Client()
        c.login(username='phil', password='adminadmin')
        ...
        response = c.post(reverse('url_riskinstance'), data=post_data, follow=True,
    ...
    def test_endpoint_url_riskinstance_post_existing(self):
        '''
        This tests that a a call to this endpoint updates
        an existing risk instance
        '''
        c = Client()
        c.login(username='phil', password='adminadmin')
        riskinstance_id = RiskInstance.objects.get(title='Property_10').id
    ...
    def test_endpoint_url_riskinstance_delete(self):
        '''
        This tests that a a call to this endpoint deletes an
        existing risk instance
        '''
        c = Client()
        c.login(username='phil', password='adminadmin')
